...
#par = np.c_[par,np.ones(par.shape[0])]
weight = np.zeros((5,28))
#weight = np.random.rand(5,28)
#print(par.shape)# -> (12960, 28)
#print(lab.shape)# -> (12960, 5)
...
if lrs == '1':
    ...
    for iterr in range(max_iter):
        ...
        for i in range(int(par.shape[0]/batch)+1):
            if par[i*batch:(i + 1)*batch,:].shape[0] != 0:
                batch_par = par[i*batch:(i + 1)*batch,:]
                batch_lab = lab[i*batch:(i + 1)*batch,:]
                a = np.dot(batch_par,np.transpose(weight))
                error = np.subtract(softmax(a), batch_lab)
                gradient = np.dot(np.transpose(error),batch_par)/(1 * batch_par.shape[0])
    ...
    for iterr in range(max_iter):
        ...
        for i in range(int(par.shape[0]/batch)):
            if par[i*batch:(i + 1)*batch,:].shape[0] != 0:
                batch_par = par[i*batch:(i + 1)*batch,:]
                batch_lab = lab[i*batch:(i + 1)*batch,:]
                a = np.dot(batch_par,np.transpose(weight))
